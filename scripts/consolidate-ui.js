/**
 * Script para consolidar componentes UI
 */
const fs = require('fs');
...
const sharedDir = 'src/shared/components/ui';
const targetDir = 'temp/components/ui';
...
Object.entries(fileMap).forEach(([fileName, paths]) => {
  ...
  if (paths.src && paths.shared) {
    ...
    if (srcStat.mtime > sharedStat.mtime) {
      console.log(`Usando versión más reciente de ${fileName} desde src/components/ui`);
      content = fs.readFileSync(paths.src, 'utf8');
    } else {
      console.log(`Usando versión más reciente de ${fileName} desde src/shared/components/ui`);
  ...
  // Si solo existe en una ubicación
    ...
    console.log(`Copiando ${fileName} desde src/components/ui`);
    content = fs.readFileSync(paths.src, 'utf8');
  } else {
    console.log(`Copiando ${fileName} desde src/shared/components/ui`);
    content = fs.readFileSync(paths.shared, 'utf8');
...
console.log(`Componentes UI consolidados en ${targetDir}`);
