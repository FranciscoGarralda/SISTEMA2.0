/**
 * Script para consolidar componentes de formularios
 */
const fs = require('fs');
...
// Directorios
const srcDir = 'src/components/forms';
const sharedDir = 'src/shared/components/forms';
const targetDir = 'temp/components/forms';
...
Object.entries(fileMap).forEach(([fileName, paths]) => {
  ...
  if (paths.src && paths.shared) {
    ...
    if (srcStat.mtime > sharedStat.mtime) {
      console.log(`Usando versión más reciente de ${fileName} desde src/components/forms`);
      content = fs.readFileSync(paths.src, 'utf8');
    } else {
      console.log(`Usando versión más reciente de ${fileName} desde src/shared/components/forms`);
  ...
  // Si solo existe en una ubicación
    ...
    console.log(`Copiando ${fileName} desde src/components/forms`);
    content = fs.readFileSync(paths.src, 'utf8');
  } else {
    console.log(`Copiando ${fileName} desde src/shared/components/forms`);
    content = fs.readFileSync(paths.shared, 'utf8');
...
console.log(`Componentes de formularios consolidados en ${targetDir}`);
console.log(`Total de componentes procesados: ${Object.keys(fileMap).length}`);
